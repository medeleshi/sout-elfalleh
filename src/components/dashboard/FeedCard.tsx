...
export default function FeedCard({ item }: FeedCardProps) {
  ...
  const href = isListing ? `/listings/${item.id}` : `/purchase-requests/${item.id}`;
  
  // Metadata extraction
  const governorate = item.governorates?.name_ar || 'غير محدد';
  ...
  // Format price/budget
  const value = isListing ? item.price : item.budget;
  const formattedValue = value ? new Intl.NumberFormat('ar-TN', { 
    style: 'currency', 
    currency: 'TND' 
  }).format(Number(value)) : 'حسب الاتفاق';

  // Specific metadata
  const quantity = isListing ? item.quantity : item.requested_quantity;
      ...
      <article className="bg-surface rounded-[2rem] border border-outline-variant/10 p-5 transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5 hover:border-primary/20 relative overflow-hidden flex flex-col sm:flex-row gap-6">
        ...
        <div className="relative w-full sm:w-48 aspect-square sm:aspect-auto sm:h-auto shrink-0 rounded-2xl overflow-hidden bg-surface-container-low border border-outline-variant/5">
          {isListing && item.listing_images?.[0] ? (
            ...
            <div className={`w-full h-full flex flex-col items-center justify-center gap-2 ${isListing ? 'bg-primary/5 text-primary' : 'bg-secondary/5 text-secondary'}`}>
              {isListing ? <Tag className="w-10 h-10 opacity-40" /> : <ShoppingBag className="w-10 h-10 opacity-40" />}
              <span className="text-[10px] font-bold uppercase tracking-widest opacity-40 text-center px-2">
                {isListing ? 'صورة العرض' : 'طلب شراء'}
              </span>
            </div>
          )}
          
          {/* Item Type Badge Overlay */}
          <div className={`absolute top-3 right-3 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-1.5 shadow-sm backdrop-blur-md border border-white/10 ${isListing ? 'bg-primary/90 text-on-primary' : 'bg-secondary/90 text-on-secondary'}`}>
            {isListing ? <Tag className="w-3 h-3" /> : <ShoppingBag className="w-3 h-3" />}
            <span>{isListing ? 'عرض للبيع' : 'طلب شراء'}</span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-between py-1 text-right">
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 text-[11px] font-bold text-on-surface-variant/70">
              ...
              <div className="flex items-center gap-2 lg:col-span-1 col-span-2">
